...
typedef AAFCOUTPUT (*AAFCExport)(float* samples, unsigned int freq, unsigned char channels, unsigned int samplelength, unsigned char bps, unsigned char sampletype, bool forcemono, unsigned int samplerateoverride, bool normalize, float pitch, bool nointerp, unsigned int lstart, unsigned int lend);
...
AAFCOUTPUT ExportAAFC(float* samples, unsigned int freq, unsigned char channels, unsigned int samplelength, unsigned char bps, unsigned char sampletype, bool forcemono, unsigned int samplerateoverride, bool normalize, float pitch, bool nointerp, unsigned int lstart, unsigned int lend) {
    AAFCExport aexport = (AAFCExport)LoadAAFCFunc(afh, "aafc_export");
    if (aexport == NULL) {
        perror("Could not initialize AAFC functions.");
        return (AAFCOUTPUT){0, NULL};
    }
    return aexport(samples, freq, channels, samplelength, bps, sampletype, forcemono, samplerateoverride, normalize, pitch, nointerp, lstart, lend);
}
